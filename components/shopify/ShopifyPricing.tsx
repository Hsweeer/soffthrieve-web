...
import { SHOPIFY_PRICING, SHOPIFY_PRICING_AR } from "@/lib/shopify-landing";
import { useDictionary, useLocale } from "@/lib/i18n";

export function ShopifyPricing() {
  const { dict } = useDictionary();
  const locale = useLocale();
  const packages = locale === "ar" ? SHOPIFY_PRICING_AR : SHOPIFY_PRICING;

  return (
    <section className="bg-[#f8f9fc] py-16 sm:py-20">
      <div className="container-shell">
        <div className="max-w-2xl">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[#2090f0]">{dict.shopify.pricingEyebrow}</p>
          <h2 className="mt-3 text-3xl font-black text-[#08112b] sm:text-4xl">{dict.shopify.pricingTitle}</h2>
          <p className="mt-3 text-sm font-semibold text-[#5a6d90]">{dict.shopify.pricingNote}</p>
        </div>
