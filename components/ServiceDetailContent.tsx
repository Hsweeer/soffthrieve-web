...
export function ServiceDetailContent({ slug }: { slug: string }) {
  ...
  const related = caseStudies
    .filter(
      (study) =>
        study.services.some((name) => name.toLowerCase().includes(service.slug.split("-")[0])) ||
        study.summary.toLowerCase().includes(service.slug.split("-")[0])
    )
    .slice(0, 3);
  ...
  return (
    <>
      <section className="overflow-hidden bg-[#f8f9fc] py-12 sm:py-16 lg:py-20">
        <div className="container-shell grid min-w-0 gap-8 sm:gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div className="scroll-reveal">
            <p className="text-sm font-black uppercase tracking-[0.18em] text-[#2090f0]">{service.eyebrow}</p>
            <h1 className="mt-4 text-3xl font-black leading-tight tracking-[0] text-[#08112b] sm:text-5xl lg:text-6xl">
              {service.title}
            </h1>
            <p className="mt-5 text-lg leading-8 text-[#5a6d90]">{service.description}</p>
            <ServiceCtaBar locale={locale} proofHref="/work" />
          </div>
      ...
      <section className="py-20">
        <div className="container-shell">
          <SectionHeading
            eyebrow={dict.services.relatedWork}
            title={`${dict.services.proofFor} ${service.title.toLowerCase()}.`}
          />
